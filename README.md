...
- **灵感库**：56 张参考图，按风格归档并记录值得学习的点；网页端可直接**上传新图**（进「待分类」等 AI 归档）、**删除**、开管理模式批量整理
- **收藏夹**：两级自建文件夹（品牌 / 主题系列 / 自定义），一张图可属于多个收藏夹；同套图可**合并为系列**叠放显示；每个风格下还能自定义**二级分类**
- **风格体系**：可复用的色板、字体、版式、效果与 AI 提示词
- **模板工坊**：编辑文字、颜色和图片，导出 2x PNG；画布旁显示**参考原图**便于对照
- **艺术工具**：三个左侧调参、右侧实时预览的图像效果工具——多主体标本画布（自动分割主体 + 半调网点图鉴）、技术线稿生成器（工程图纸感生成式海报，导出 SVG/PNG）、图文语义混排（大标题绕排避让主体的实验排版）；支持挂载参考原图
- **REST API**：读取风格、灵感、模板和收藏夹，支持受保护的新增、修改、删除与图片上传
- **可靠回退**：API 不可用时，前端自动使用仓库内置数据（只读）
...
├── js/tools/index.js      # 艺术工具注册表与工具箱首页
├── js/tools/shared.js     # 共享算法：主体分割/半调/描边/导出/控件
├── js/tools/specimen.js   # 工具：多主体标本画布
├── js/tools/techlines.js  # 工具：技术线稿生成器
├── js/tools/typeflow.js   # 工具：图文语义混排
